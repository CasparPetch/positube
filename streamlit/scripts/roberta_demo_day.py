...
def roberta_channel(BIGdf):
    ...
    for i, df in cut_dfs:
        results = sentiment_score_comment(df)
        positivity_score = results['weighted_SV'].mean()
        IDs_df['positivity_score'][i] = positivity_score
        results['video_id'] = df['video_id']
        results_list.append(results)
        # pd.concat(results_list).to_csv("results.csv")
    ...
    for result in results_list:
        pos_dict['video_id'] = result['video_id']
        pos_dict['positivity_score'] = result['weighted_SV'].mean()

    pos_score_df = pd.DataFrame.from_dict(pos_dict)

    return comment_score_df, pos_score_df
