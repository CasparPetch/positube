# my function imports
from single_video_scripts.fetching_details import fetch_details
from single_video_scripts.fetch_channel import fetch_channel
from single_video_scripts.fetch_stats import fetch_stats
from single_video_scripts.comments_relevance import fetch_comments_relevance

# from fetching_details import fetch_details
# from fetch_channel import fetch_channel
# from fetch_stats import fetch_stats
# from comments_relevance import fetch_comments_relevance
import pandas as pd

# temporary
from icecream import ic
ic.disable()

def fetch_all_channel_info(df, api_key):
    all_comments = []
    all_infos = []
    for video_id in df['video_id']:
        # fetch the details of the individual video
        details = fetch_details(video_id, api_key)
        ic.enable()
        # testing fetch_comments_relevance it works!
        comments_relevance = pd.DataFrame(fetch_comments_relevance(video_id, api_key))
        # testing fetch_stats and it works!
        stats = pd.DataFrame(fetch_stats(video_id, api_key))
        ic(details['channel_id'])
        ic.disable()
        ic(stats)
        ic(comments_relevance)
        # testing fetch_channel and it works!
        channel = pd.DataFrame([fetch_channel(details['channel_id'], api_key)])
        info_all = pd.concat([stats,pd.DataFrame(details)],axis=1)
        ic(info_all.head(2))
        comments_relevance["video_id"] = video_id
        info_all["video_id"] = video_id
        ic(comments_relevance.columns)
        ic(type(comments_relevance))
        ic(type(comments_relevance.at[2, 'comment_clean']))
        all_comments.append(comments_relevance)
        all_infos.append(info_all)
    return pd.concat(all_comments), pd.concat(all_infos), channel

if __name__ == '__main__':
    import os
    test_video = 'SGqg_ZzThDU'
    API_KEY = os.getenv('API_KEY')
    fetch_all_channel_info(test_video, API_KEY)
